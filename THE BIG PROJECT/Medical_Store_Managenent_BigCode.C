...
COORD coord = {0, 0};
void gotoxy(int x, int y)/*gotoxy is a function that locates the text cursor to x,y positions on 
the screen. Any outputs to the screen will 
start at the cursor x,y positions, and the cursor is updated accordingly.*/
{
 COORD coord;
 coord.X = x;
 coord.Y = y;
...
//   window(1,1,80,50);
	...
	gotoxy(10,8);
	printf("=> Project Is About Medical Store ");
	gotoxy(10,10);
	printf("=> User Can Add Medicine ,Customer,Supplier Details");
	gotoxy(10,12);
	printf("=> User Can Modify and Delete Existing Record");
	gotoxy(10,14);
	printf("=> User Can Search Medicine ,Customer,Supplier Details");
	gotoxy(10,16);
	printf("=> Also Used For Displaying Stock Of Medicine ");
...
//		t();  // THIS FUN IS USED FOR DISPLAY DATE & TIME.......
		...
		printf(" MEDICINE PURSHASE  ");
		{
			...
			gotoxy(7,27);
			printf("MFG.txtE(dd-mm-yyyy): ");

			gotoxy(7,29);
			printf("EXP.txtE(dd-mm-yyyy): ");
...
void stock()
{
  ...
  int i,c;
	   do
	   {
		...
		i=14;
		gotoxy(9,10);
		printf("ID.   MEDICINE NAME.    QTY     Supplier Name     Exp.txte");
		gotoxy(9,12);
		printf("==================================================================\n");
